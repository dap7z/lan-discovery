...
WinParser.prototype._processBody = function (line) {
    ...
    // kvps.length >= 3 means target is pingable, example [ 'octets=32', 'temps=243', 'TTL=64' ]
    if (kvps.length >= 3) {

        // XXX: Assume time will always get keyword ms for all language
        var regExp = /([0-9\. ]+)ms/;
        var match = regExp.exec(line);
        var result = (match ? match[1].trim() : null)

        this._times.push(parseFloat(result, 10));
    }
